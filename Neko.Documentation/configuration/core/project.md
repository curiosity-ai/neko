...
| `blog` | The marketing-site look (as on [curiosity.ai](https://curiosity.ai)): a borderless header with the nav links clustered next to the logo (used on its own as a wordmark) and the [`actions`](#actions) CTA buttons pushed to the right, plus an edge-to-edge [marketing footer](#footer). The page/header/CTA palette comes from [`theme.base`](#theme-base). Blog mode is **light-only by default** (the toggle is hidden and light is locked); define a [`theme.dark`](#theme-dark) palette to opt into dark mode and bring the toggle back. The site search moves out of the header to a bar at the top of the post list and searches **inline** — typing filters the index live and renders the matching **blog posts only** (with their cover thumbnail and tags, no breadcrumb trail) in place of the post grid, rather than opening the `⌘K` modal. A row of **tag chips** under the search bar filters the post grid by tag with an empty query: the matching posts stay as cards, and an **All** chip clears the filter. Each post card also shows its own tags as pills, so the bucket is visible at a glance. |
