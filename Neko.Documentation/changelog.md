...
* **Feature**: Added a new `Roadmap` component that renders a kanban-style product roadmap board. Lanes (`:::: lane`) carry a title, count badge and accent colour; items (`::: roadmap-item`) carry a title, tag pill, optional date, vote count, and optional clickable link. Documentation lives at `components/roadmap.md`, with three `neko-roadmap*` snippets in `templates.json`.
