...
* **Feature**: Documented and tested the `>>>` **Steps** component &mdash; a numbered sequence of titled steps with circle markers and a vertical guide line. Added a dedicated `components/steps.md` page, fixed the `neko-steps` editor snippet to use the actual `>>> Title` syntax, and added a `StepsTests` suite covering numbering, group wrapping, inline-markdown titles, and block-level bodies.
