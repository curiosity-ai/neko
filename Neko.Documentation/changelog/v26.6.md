...
**persisted to disk** and reused for 24 hours, so a fresh `neko watch` no longer
makes a NuGet round-trip before serving cached samples, and a new Tesserae
release no longer silently invalidates every cached sample mid-session. Pin an
exact version with `tesserae.version` in `neko.yml` for fully deterministic
builds. When NuGet is unreachable, the last known version is reused instead of
failing the build. The shared H5 runtime (h5.js, css, …) is also stored once per
version rather than once per sample.
:::
