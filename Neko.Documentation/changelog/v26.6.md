...
::: change {badge="Improved" title="Blog-mode header has a mobile menu and never squeezes the logo"}
...
::: change {badge="New" title="Blog index hero — pill, title & lead"}
The `mode: blog` index page can now lead with a marketing hero above the search
bar and post grid, matching curiosity.ai/resources/blog: a rounded **pill**, a
large **title**, and an optional **lead** paragraph, configured under a new
top-level `blog:` block (`pill` / `title` / `lead`). When `title` is set it
replaces the plain label `<h1>` the index would otherwise render. The pill uses
a plain system sans-serif at 12px while the title inherits the site body font
(`theme.font`) at 30.4px / weight 500, both picking up blog mode's antialiased
smoothing; colours track the `theme.base` palette. The post-card grid also gets
a roomier **44px gutter** between cards. Documented at
`configuration/core/project.md#blog` and the `neko-yml` skill.
:::

::: change {badge="Improved" title="Blog index & filters view polish"}
...
::: change {badge="Improved" title="Wider blog index for the card grid + search"}
...
column, so article line length is unchanged.
:::

::: change {badge="Improved" title="Animated arrow on blog post cards"}
Each blog post card carries a small right-pointing arrow that **rotates 45° on
hover** (a 300ms ease-out transform), swinging from → to ↗ — the
curiosity.ai/resources/blog interaction. It pairs with the existing cover-image
zoom for the same lift-on-hover feel.
:::

::: change {badge="Improved" title="Blog covers always reserve a consistent slot"}
...
::: change {badge="Improved" title="Blog-mode header is a pinned overlay topbar"}
...
::: change {badge="Improved" title="Blog post cards show their tags"}
...
::: change {badge="Fixed" title="No console error in blog mode without a dark theme"}
...
threw `TypeError: can't access property "addEventListener", themeToggleBtn is
null` on every page load — aborting the rest of the inline script. The listener
