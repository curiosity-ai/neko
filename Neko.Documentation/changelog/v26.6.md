...
::: change {badge="New" title="Configurable blog index hero (pill + title)"}
...
the blog index in `mode: blog`; when all fields are unset the index opens straight
into the search box, as before. The fields are inherited per-field by multi-repo
child sites. Documented at `configuration/core/project.md#blog` and the `neko-yml`
skill.
:::
...
The up-right arrow on each blog post card now **nudges diagonally up-and-right
on hover** (a 300ms ease-out transform), matching the
