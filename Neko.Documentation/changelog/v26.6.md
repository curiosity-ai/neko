...
::: change {badge="Improved" title="Blog header and footer match curiosity.ai exactly"}
Blog mode's chrome now mirrors the [curiosity.ai](https://curiosity.ai) blog
pixel-for-pixel. The header **clusters the nav links immediately to the right of
the logo** (instead of centring them between the logo and the CTA buttons), with
the `actions` pills pushed to the far right. The marketing footer is now
**edge-to-edge**: the dark rounded panel spans the full content pane to both
screen edges, while its inner content stays centred at `layout.maxWidth` so the
footer logo lines up under the header logo. This is achieved by not capping the
content row in blog mode — the header and post grid keep their own centring — so
the footer can break out to the pane edges. `docs` mode is unchanged.
:::
