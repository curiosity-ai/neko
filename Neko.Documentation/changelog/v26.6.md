...
The header **clusters the nav links immediately to the right of the logo**
(instead of centring them between the logo and the CTA buttons), at 15px /
weight 400, with the `actions` pills pushed to the far right. The marketing
...
at `layout.maxWidth` so the footer logo lines up under the header logo, with
near-white 15px link columns. This is achieved by not capping the content row in
blog mode — the header and post grid keep their own centring — so the footer can
break out to the pane edges. `docs` mode is unchanged.
:::
...
::: change {badge="Fixed" title="Protected pages don't flash an empty view transition"}
Navigating to a password-protected page with the cross-document View Transitions
API enabled briefly showed a **blank page with a collapsed sidebar** before the
real content appeared. The transition snapshots the incoming page at its first
paint — but a protected page ships its content encrypted and its protected
sidebar entries hidden, so at that moment the page is genuinely empty;
`password.js` only decrypts and reveals everything a frame or more later, after
the cross-fade had already animated to the empty snapshot. Protected pages now
**skip the view transition** (on the `pagereveal` event) so they load plainly and
`password.js` fills them in without an intervening empty frame. The sidebar
scroll position is also re-applied **after** the protected entries are revealed —
the inline restore ran while they were still hidden, so the sidebar was too short
for the saved `scrollTop` to take effect and snapped back to the top once the
entries appeared.
:::
