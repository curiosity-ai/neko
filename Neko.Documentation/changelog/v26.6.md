...
::: change {badge="New" title="Pin Tesserae preview heights with `gen-tesserae-heights`"}
A `tesserae` block can carry a `height=<px>` argument
(` ```tesserae chrome="macos" demo.js height=420 `) that pins its live-preview
iframe to that height; without it the iframe keeps the resizable 400px minimum.
A new `neko gen-tesserae-heights` command measures the rendered height of each
sample and writes the argument back onto the block. Scope it to one file with
`--file <path>` and rerun it after editing a sample — it is file-targeted with
no hash cache, so the file you target is exactly what gets regenerated.
Measurement uses the same Playwright/snapframe harness as `neko snap`.
:::
