...
in the muted `#f1f1f1` ink with `capitalize`, link text uses the `#b4b9b1`
...
::: change {badge="Improved" title="Blog-mode footer badges and spacing pixel-match curiosity.ai"}
Follow-up polish on the blog-mode marketing footer to line it up with
curiosity.ai down to the pixel. The trust-badge thumbnails are now **circular**
(`rounded-full`, `object-cover`) like the live badges instead of square, the
badge column uses the marketing site's looser vertical rhythm (~78px between
badges), and the link columns pick up the same 37px row pitch. The gap before the
copyright bar was retuned so the panel keeps its 526px height with the looser
spacing.
:::
