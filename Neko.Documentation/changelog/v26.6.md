...
(nav + CTA pills) renders in **Inter** even when `theme.font` sets a brand font
for the content and footer. The `actions` pills are pushed to the far right. The marketing
...
with keyboard navigation (↑/↓/↵) and `Esc` to clear. Each result now shows the
page's **tags** as chips, with any tag matching the query highlighted, so a search
like `release` surfaces every post tagged that way at a glance. Tags are exported
as a discrete `tags` field in `search.json` (in addition to being full-text
searchable), and the chips also appear in the docs-mode `⌘K` modal. `docs` mode
keeps its modal search. Documented at `configuration/core/project.md#mode`.
:::
...
A new `theme.font` setting overrides the site's base typeface. By default Neko
loads **Inter**; set `theme.font.family` (and an optional `theme.font.url`
stylesheet — a Google Fonts link, a CDN, or a self-hosted `/assets/….css`) to
render the whole site — header, content, and footer — in your brand font. For
example, `family: Plus Jakarta Sans` reproduces the curiosity.ai blog
typography. The family is used as the base `font-family` with a `, sans-serif`
fallback; a full comma-separated stack is accepted verbatim. `theme.font` is
inherited by multi-repo child sites. Documented at
`configuration/core/project.md#theme-font` and the `neko-yml` skill.
:::
