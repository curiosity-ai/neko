...
The left-sidebar navigation no longer renders an icon next to every page and
folder by default — labels sit flush left for a cleaner tree. Icons are now
controlled by a new `nav.icons.mode` setting that defaults to `none`. Whitelist
where they appear by setting it to `all`, `folders` (alias `parents` — items
that have sub-pages), `pages`, or `top`:
...
A page's or folder's configured `icon:` is still used elsewhere (top nav,
breadcrumbs) — this setting only governs the left sidebar. Documented at
