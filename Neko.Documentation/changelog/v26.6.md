...
::: change {badge="Fixed" title="Sidebar no longer re-renders after each navigation"}
The left navigation used to visibly "settle" on every page load — the active
item highlighted a beat late, open/closed sections snapped into place, and (on
protected sites) titles appeared — because all of that was applied at the end of
`<body>`, after the menu had already painted. That state is now restored
synchronously *before* first paint, immediately after the sidebar markup, so the
menu renders in its final form on the first frame. Saved scroll position is
restored the same way (best effort, since it depends on layout).
:::

::: change {badge="Fixed" title="No more white flash between pages on the dark theme"}
Clicking from one page to the next no longer shows a brief white blink. The
`<html>` element had no background of its own (only `<body>` did, and only once
the JS-applied `dark` class landed), so the browser's default white canvas showed
through during the navigation gap — very visible against a dark site. The theme
background is now painted on `<html>` directly and `color-scheme` is set on the
root in the pre-paint theme script, so the canvas is the right colour from the
first frame.
:::

::: change {badge="Fixed" title="Password gate no longer flashes when navigating between protected pages"}
A visitor who has already unlocked a protected site (or page) no longer sees the
"Password Protected" form flash on every page-to-page navigation. The gate is
server-rendered, so it used to paint before the client-side auto-unlock could
decrypt and swap in the content. Now a synchronous inline script hides the gate
before first paint whenever a matching password is saved in `sessionStorage`,
so the content swaps straight in once decryption resolves (~25ms). As part of the same fix, an
*automatic* unlock that fails (e.g. a stale saved password after the password
changed) now reveals a clean password form instead of a spurious "Incorrect
password" error the reader never triggered.
:::

::: change {badge="Fixed" title="Protected sidebar no longer flashes empty on every navigation"}
On a globally-protected site the sidebar titles are encrypted too, so the whole
left nav used to paint **empty** on every page load and then trickle its entries
back in as the browser decrypted them one by one — each title ran its own PBKDF2
key derivation (~100k iterations), so a few dozen entries cost the better part of
a second per navigation. Two changes fix this. First, all protected payloads on a
page now share a single salt, so the browser derives the key once and reuses it
for the page body and every sidebar title (the derivation is cached and
de-duplicated across the concurrent body/sidebar unlocks) — on a 38-payload
academy page that cut client-side decryption from ~700&nbsp;ms to ~25&nbsp;ms.
Second, once decrypted, the sidebar titles are cached in `sessionStorage` and
restored **synchronously, before first paint**, on subsequent navigations — so
the sidebar renders complete immediately instead of flashing. AES-GCM stays safe
because every payload keeps its own random nonce.
:::
...
::: change {badge="New" title="Tailor the displayed source of Tesserae samples"}
A ` ```tesserae ` block can now fence lines with two marker pairs so the source
shown in the **Code** tab differs from what runs. `// <hide>` … `// </hide>` is
compiled and run but removed from the Code tab — for styling, layout chrome, or
demo-only plumbing. `// <docs>` … `// </docs>` is shown in the Code tab but not
compiled — for displaying the idiomatic call a sample can't run as-is (e.g. an
API the sandboxed preview can't use) while a hidden block runs a preview-safe
variant. Both the cache-warming and render passes apply the same stripping, so a
sample compiles once. Documented at `components/tesserae.md` and in the
`tesserae` skill.
:::
