...
project the result belongs to. The name resolves from a new optional
`breadcrumb.label` key, falling back to `branding.label`, then `branding.title`,
then a title-cased mount path:
...
Documented at `configuration/core/project.md#breadcrumb`.
:::
