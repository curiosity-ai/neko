...
The pinned version header on the changelog timeline was a rounded, inset box that
stuck one scroll-padding step below the top, so entries scrolling past it peeked
out around its rounded corners and through the strip above it. The header now
pins flush against the top of the scroll area as an edge-to-edge glassy bar that
spans the full content pane, while its version pill and date stay aligned with
the release body — so the current release's entries slide cleanly underneath it
as you scroll.
:::
