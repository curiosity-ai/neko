...
that render as a dark, rounded marketing footer in blog mode. `mode` defaults to
`docs`, so existing sites are unchanged, and it is inherited by multi-repo child
sites. Documented under `configuration/core/project` (`mode`, `actions`, `footer`)
