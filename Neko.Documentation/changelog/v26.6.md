...
Utility classes used only inside a password-protected page (`password:` in the
frontmatter, or a site-wide `password:` in `neko.yml`) were dropped from the
generated `tailwind.css`, so the decrypted page rendered unstyled. Protected
pages are written to disk as an encrypted blob, and the Tailwind class extractor
— which scans the emitted HTML — never saw the classes inside them. The most
visible symptom was an alert (`!!!`) on a protected page losing its dark-mode
colours: a washed-out, low-contrast box. The builder now captures each protected
page's plaintext at render time and feeds it to the extractor, so those classes
are emitted like any other.
:::
