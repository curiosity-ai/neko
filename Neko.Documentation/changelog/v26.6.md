...
::: change {badge="New" title="Blog-mode \"Read next\" section and CTA band"}
...
::: change {badge="Improved" title="Wider blog content column"}
Blog-mode pages now use a roomier `max-w-6xl` (~1152px) content column instead
of the `max-w-4xl` reading width — both the **index** (so the post-card grid and
search box get more breathing room) and the **article** pages (so the post body
lines up with the Read next / CTA card grid below it). Matches the wider
curiosity.ai/resources/blog layout. Docs-mode pages keep the comfortable
`max-w-4xl` reading column.
:::

::: change {badge="Improved" title="Animated arrow on blog post cards"}
Each blog post card carries a small right-pointing arrow that **rotates 45°
counter-clockwise on hover** (a 300ms ease-out transform), swinging from → to ↗
— the curiosity.ai/resources/blog interaction. It pairs with the existing cover-image
...
::: change {badge="Fixed" title="Blog post covers resolve and tolerate spaces in the path"}
