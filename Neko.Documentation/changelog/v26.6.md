...
sub-project's own sidebar, so a page at `landlock-sharp/advanced/testing`
showed only `Advanced` — dropping the project segment that distinguishes it
from a same-named section in another sub-site. The route-prefix segments (the
sub-project's mount path, e.g. `landlock-sharp`) are now prepended to every
indexed page and section, so the first crumb is always the project the result
belongs to.
:::
...
::: change {badge="Fixed" title="Unhighlighted code tokens no longer wash out"}
Plain tokens in a code block — identifiers, type names, punctuation, braces —
could render in a faint grey on the light card, while keywords and other
highlighted spans stayed correctly coloured. Tailwind Typography styles
`<pre>`/`<code>` with low-specificity `:where()` rules at the same weight as the
highlight theme's `.hljs`, so whichever stylesheet loaded last won; when the
prose rule won, the unhighlighted text took prose's pre-code colour (a light
grey meant for prose's dark `<pre>`). The code-block now re-asserts the active
theme's base text colour at higher specificity in both modes
(`.neko-code-block pre code.hljs`), so plain tokens stay legible regardless of
stylesheet order while highlighted `.hljs-*` spans keep their own colours.
:::
