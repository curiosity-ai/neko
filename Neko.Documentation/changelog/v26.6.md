...
::: change {badge="Improved" title="C# docs group method overloads"}
The `csharp-docs` block now renders overloads — members that share a name but
differ in signature — as a **single entry** instead of repeating the method once
per overload. The set gets one header and permalink anchor (the plain name, e.g.
`#Client.Connect`), every signature stacked in one code box, and one shared
description: the standard `<overloads>` XML tag when present, otherwise the first
overload's `<summary>`. Parameters are merged into a **union list** — those common
to every overload appear once, while a parameter that belongs to only some of them
is tagged `(overload N)` — and any per-overload `<summary>` that differs from the
shared lead is kept in a short **Overloads** list, so nothing is lost. The member
summary table lists the set once. Documented at `components/csharp-docs.md`.
:::
