...
A top-navigation link group can now set `pivot: true` to surface its `items` as
a row of tabs in a secondary bar directly below the header. The bar appears only
while the reader is inside that section — a page is "inside" when its URL matches
one of the group's item links (or sits below it) — and the tab for the current
page is highlighted. Several groups can each be a pivot; the one whose item most
specifically matches the current page wins. The existing flyout dropdown still
works, so a single group can act as both a header dropdown for jumping between
sections and an in-section pivot. Documented at
`configuration/core/project.md#pivot`.
:::
