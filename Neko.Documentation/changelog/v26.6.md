...
::: change {badge="Fixed" title="Sticky changelog version header spans the full width"}
The pinned version header on the changelog timeline was a rounded, inset box, so
while it stayed stuck at the top, entries scrolling past it peeked out around its
rounded corners and to its left over the timeline rail. The header's glassy
background now spans the full width of the timeline and sits flush at the top,
with a hairline bottom border, so the current release's entries slide cleanly
