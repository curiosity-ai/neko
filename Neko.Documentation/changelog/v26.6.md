...
::: change {badge="New" title="Filter the blog by tag"}
The `blog`-mode post list now renders a **"Filter by" pill row** below the search
bar, built automatically from the union of every post's `tags` (ordered by how
many posts carry each tag). Clicking a pill narrows the grid to posts with that
tag and clicking it again clears the filter — no configuration and no extra
markup, the row simply reflects whatever tags the posts use. Each post card also
shows its own tags, so a reader can see which bucket a post belongs to at a
glance. The filtering is done client-side off a `data-tags` attribute on each
card, so it works on a static build with no backend. Documented at
`configuration/core/project.md#mode`.
:::
