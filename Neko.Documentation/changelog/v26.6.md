...
::: change {badge="Fixed" title="Sub-project intro pages no longer mislabelled \"Home\""}
...
sub-site's intro page was labelled "Home" and pointed at the sub-site root,
duplicating the highlighted pivot tab that already links there. Only the **true
site root** (no route prefix) is now renamed to "Home"; a sub-project root keeps
its own `label`/`title` (e.g. "Introduction") instead.
:::
