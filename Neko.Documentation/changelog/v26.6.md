...
::: change {badge="Fixed" title="Tesserae live preview loaded from a real URL — History API works"}
The live-preview pane of a ` ```tesserae ` block was inlined into the iframe with
`srcdoc`, which gives the document the URL `about:srcdoc`. The History API is
unusable from such a document: `history.pushState`/`replaceState` throw a
`SecurityError`, so any sample that navigates (for example Tesserae's `Router`)
broke as soon as a button was clicked. The compiled preview is now written to a
file under `assets/tesserae/previews/` and the iframe loads it through `src`, so
the sample runs from a real same-origin URL and routing samples work. The
compile-failure placeholder still uses `srcdoc`.
:::
