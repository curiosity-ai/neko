...
::: change {badge="Improved" title="Tesserae previews reserve the right height — no layout shift"}
` ```tesserae ` live previews no longer reflow the page when they finish
rendering. Each preview used a fixed `400px` placeholder iframe, so short samples
left dead space and tall ones were clipped behind an inner scrollbar — and the
page jumped as samples loaded. Neko now measures every compiled sample's rendered
height with a headless browser ([snapframe](/components/snapframe)/Playwright)
during the cache-warming pass and bakes that exact height into the iframe, so the
right space is reserved up front. The measurement is stored in the sample cache
alongside the compiled output, so the browser runs **once per unique sample**.
Turn it off with `tesserae.measureHeight: false` for offline builds with no
browser toolchain (iframes fall back to the placeholder height), and tune the
measurement viewport with `tesserae.measureWidth`. Documented at
`configuration/core/project.md#tesserae`.
:::
