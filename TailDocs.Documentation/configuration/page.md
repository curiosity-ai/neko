...
If you add one or more configs to a `.md` page, TailDocs will not look for nor read the separate **.yml** file. Just use one or the other, but not both.
!!!
...
Individual category summary pages will be automatically generated by TailDocs at `<url>/categories/<category>`, plus an additional `<url>/categories` index page which lists all categories in the project.
...
If a `date` is configured, TailDocs will add the **Published** date to the top of the page, just under the main title.
...
The `date` is used by TailDocs to order blog pages. Newer blog pages are ordered first.
...
By default, TailDocs will try to find the first image on the page and use that image as the feature image to highlight when creating a summary of the page.
...
If set, the value will completely override any `<title>` meta tag value generated by TailDocs.
...
If the `order` is set with a number, a larger positive number will give more _weight_ or _priority_ to that page and TailDocs will _bubble up_ that page in the navigation. For instance, a page configured with `order: 100` will be higher in the navigation than a page configured with `order: 10`.
...
A custom URL path for this page or folder which overrides the default path generated by TailDocs.
...
Configuring the `permalink` is an excellent solution when moving to TailDocs from another platform and you would like to maintain existing public URL's, but would prefer to re-organize your `.md` files into a new structure. A `permalink` allows for a clean disconnect of the page path from the final published URL path.

In the following sample, the generated URL by TailDocs would be `/guide/2021-06-25-publishing-to-github-pages/`, but we override with a custom permalink which will publish the page to `/tutorial/publish-to-github-pages/`.
...
TailDocs will automatically handle any incoming **example.com/setup** requests and redirect to the new **example.com/getting-started** location.

TailDocs is also smart enough to scan your project for any **setup.md** links and replace those with a link directly to **getting-started.md**.
...
Individual tag summary pages will be automatically generated by TailDocs at [`<url>/tags/<tag>`](/tags/guide/), plus an additional [`<url>/tags`](/tags/) index page which lists all tags and their count used within the project.
...
TailDocs automatically transforms common values like `blank` to `_blank`.
...
The `title` config instructs TailDocs to add a primary `# Page Title` to your page, but it is recommended to manually add a `# Page Title` to each of your pages, instead of setting a `title`.
...
TailDocs would ensure no automatically generated links or references to the hidden page are created. If on a public page, a link to a hidden page is made by an author, the link will work and the hidden page will be visible.
