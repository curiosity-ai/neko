    ...
    function renderResults(results, container) {
        ...
        container.innerHTML = results.map((result) => {
            ...
            const breadcrumb = result.type === 'section' && result.parentTitle
                ? `<div class="text-[11px] uppercase tracking-wide text-gray-400 dark:text-gray-500 mb-0.5">${escapeHtml(result.parentTitle)}</div>`
                : '';
            ...
            <a href="${href}" class="block px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700/50 group">
                ...
                ${snippet ? `<div class="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">${snippet}</div>` : ''}
