...
- **IMPORTANT**: When generating the documentation site locally for testing or verification using `dotnet run`, **always use a temporary output folder** (e.g., `test_out/`).
- **NEVER** push or commit temporary files, logs, or generated documentation files (like `test_out/`, `test_dir/`, or any `.html` files in the repository root) to the codebase. Ensure the working tree is clean of test artifacts before committing.